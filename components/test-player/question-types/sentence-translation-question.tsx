...
export default function SentenceTranslationQuestionRenderer({
  ...
  const aiScores = useMemo(() => {
    const scores: Record<
      string,
      { score: number; feedback: string; maxScore: number }
    > = {};
    ...
    if (answer && typeof answer === "object") {
      ...
      Object.values(answer).forEach(
        (answerEntry: {
          ...
          if (
            answerEntry?.questionId === question.id &&
            answerEntry?.subQuestionId
          ) {
      ...
      <div className="space-y-6">
        {sentences.map((sentence, index) => {
          ...
          const isExactMatch =
            showCorrectAnswer &&
            userAnswer &&
            sentence.referenceTranslations?.some(
              (ref) =>
          ...
          const isAICorrect =
            showCorrectAnswer && score && score / maxScore >= 0.8;
                ...
                <div className="p-4">
                  <div className="space-y-2">
                    ...
                    <Textarea
                      value={userAnswer}
                      onChange={(e) =>
                        handleChange(sentence.id, e.target.value)
                      }
                      placeholder={
                        showCorrectAnswer
                          ? "Not answered"
                          : `Enter your ${getLanguageLabel(
                      ...
                      <Card className="bg-blue-50/50 dark:bg-blue-900/10 border-blue-200/50 dark:border-blue-800/50">
                        <div className="p-3">
                          ...
                          <p className="text-xs text-muted-foreground mt-2">
                            This is an AI-generated evaluation. In a real test,
                            translations would be scored by human experts.
                          </p>
                        </div>
                      </Card>
                    </div>
                  ...
                  {showCorrectAnswer &&
                    isIncorrect &&
                    sentence.referenceTranslations &&
                    sentence.referenceTranslations.length > 0 && (
                      <div className="mt-3">
                        <div className="text-sm p-3 bg-green-50/50 rounded border border-green-200/50">
                          <div className="font-medium text-green-800 mb-1 flex items-center">
                            <span className="w-4 h-4 rounded-full bg-green-600 text-white text-xs flex items-center justify-center mr-2">
                              ✓
                            </span>
                            Reference translation(s):
                          </div>
                          <ul className="space-y-1 text-green-700">
                            {sentence.referenceTranslations.map(
                              (ref, refIndex) => (
                                <li key={refIndex} className="pl-6">
                                  • {ref}
                                </li>
                              )
                            )}
