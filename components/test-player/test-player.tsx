...
import { ArrowLeft, ArrowRight, CheckCircle2, BookOpen, SplitSquareVertical } from "lucide-react"
...
export default function TestPlayer({ test }: TestPlayerProps) {
  ...
  const currentGroup =
    isPartOfGroup && section.questionGroups ? section.questionGroups.find((g) => g.id === question.groupId) : undefined

  // Get all questions in the current group
  const groupQuestions = currentGroup ? section.questions.filter((q) => q.groupId === currentGroup.id) : []
    ...
    <div className="container mx-auto py-6 px-4">
      <div className="mb-6 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex items-center gap-4">
          <TestTimer initialTime={progress.timeRemaining} onTimeEnd={handleCompleteTest} />
        ...
        <div className="mb-6">
          ...
          {!audioPlayed && (
            <Alert className="mt-2">
              <AlertDescription>
                In a real IELTS test, you would only hear the recording once. Listen carefully.
              </AlertDescription>
            </Alert>
          )}
        </div>
      ...
      {isReadingTest && section.readingPassage && (
        <div className="mb-6">
          ...
          {showPassage && <ReadingPassageViewer passage={section.readingPassage} />}
        </div>
      )}
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="mb-6">
        ...
        <TabsContent value="questions">
          {isPartOfGroup && currentGroup ? (
            <QuestionGroupRenderer group={currentGroup} questions={groupQuestions} sectionId={section.id} />
          ) : (
            <Card>
              <CardHeader>
                <CardTitle className="flex justify-between items-center">
                  <span>
                    Question {progress.currentQuestionIndex + 1} of {section.questions.length}
                  </span>
                </CardTitle>
              </CardHeader>
              ...
              <CardFooter className="flex justify-between">
                <Button
                  ...
                  disabled={progress.currentSectionIndex === 0 && progress.currentQuestionIndex === 0}
                >
        ...
        <TabsContent value="section-info">
          <Card>
            ...
            <CardContent>
              ...
              {section.audioUrl && (
                <Alert className="mb-4">
                  <AlertDescription>
                    This section includes an audio recording. You will only hear the recording once.
                  </AlertDescription>
                </Alert>
              )}
              ...
              {isReadingTest && (
                <Alert className="mb-4">
                  <AlertDescription>
                    This section includes a reading passage. You can toggle the passage visibility using the button
                    above.
                  </AlertDescription>
                </Alert>
              )}
            ...
            <CardFooter>
              <Button onClick={() => setActiveTab("questions")} className="w-full">
                Return to Questions
              </Button>
            </CardFooter>
          </Card>
